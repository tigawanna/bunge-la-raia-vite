...
import { listConstituenciesQueryOptions} from "./counstituency-query-options";

interface CounstituenciesListProps {
  q: string;
}

export function CounstituenciesList({ q = "" }: CounstituenciesListProps) {
  const query = useSuspenseQuery(listConstituenciesQueryOptions({ search_query: q }));
